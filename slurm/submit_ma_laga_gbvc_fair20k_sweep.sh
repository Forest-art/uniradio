...
STEPS=${STEPS:-20000}
SEED=${SEED:-42}
RUN_PREFIX=${RUN_PREFIX:-cifar100_patch4_ma_laga_gbvc20k}
...
OUT_TSV=${OUT_TSV:-/scratch/peilab/xlubl/unirae_runs/ma_laga_gbvc20k_jobs_$(date +%Y%m%d_%H%M%S).tsv}
mkdir -p "$(dirname "${OUT_TSV}")"
echo -e "jobid\trun_name\tbase\talign_gamma\tnorm_restore\tlambda_gbvc\tgbvc_nu" > "${OUT_TSV}"

submit_one () {
  local run_name="$1"
  local base_tag="$2"
  local align_gamma="$3"
  local norm_restore="$4"
  local lambda_gbvc="$5"
  ...
  echo -e "${job_id}\t${run_name}\t${base_tag}\t${align_gamma}\t${norm_restore}\t${lambda_gbvc}\t${GBVC_NU}" >> "${OUT_TSV}"
  echo "${job_id}"
}

# Base 1: MA-LAGA_v2 (align_gamma=1.0, norm_restore=False)
r="${RUN_PREFIX}_basev2_lgb005_s${SEED}_$(date +%Y%m%d_%H%M%S)"
j=$(submit_one "${r}" "v2" "1.0" "false" "0.05")
echo "[submit_ma_laga_gbvc_fair20k] submitted ${j} ${r}"

r="${RUN_PREFIX}_basev2_lgb010_s${SEED}_$(date +%Y%m%d_%H%M%S)"
j=$(submit_one "${r}" "v2" "1.0" "false" "0.10")
echo "[submit_ma_laga_gbvc_fair20k] submitted ${j} ${r}"

r="${RUN_PREFIX}_basev2_lgb020_s${SEED}_$(date +%Y%m%d_%H%M%S)"
j=$(submit_one "${r}" "v2" "1.0" "false" "0.20")
echo "[submit_ma_laga_gbvc_fair20k] submitted ${j} ${r}"

# Base 2: MA-LAGA_v4 (align_gamma=1.0, norm_restore=True)
r="${RUN_PREFIX}_basev4_lgb005_s${SEED}_$(date +%Y%m%d_%H%M%S)"
j=$(submit_one "${r}" "v4" "1.0" "true" "0.05")
echo "[submit_ma_laga_gbvc_fair20k] submitted ${j} ${r}"

r="${RUN_PREFIX}_basev4_lgb010_s${SEED}_$(date +%Y%m%d_%H%M%S)"
j=$(submit_one "${r}" "v4" "1.0" "true" "0.10")
echo "[submit_ma_laga_gbvc_fair20k] submitted ${j} ${r}"

r="${RUN_PREFIX}_basev4_lgb020_s${SEED}_$(date +%Y%m%d_%H%M%S)"
j=$(submit_one "${r}" "v4" "1.0" "true" "0.20")
echo "[submit_ma_laga_gbvc_fair20k] submitted ${j} ${r}"

echo "[submit_ma_laga_gbvc_fair20k] job table: ${OUT_TSV}"
