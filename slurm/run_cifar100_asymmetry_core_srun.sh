...
OUT_DIR=${OUT_DIR:-${ROOT_DIR}/results/cifar100_asymmetry_core_srun_$(date +%Y%m%d_%H%M%S)}
...
HOLD_JOB_NAME=${HOLD_JOB_NAME:-codex-srun-hold}
NODES=${NODES:-1}
...
DSGA_GAMMA=${DSGA_GAMMA:-0.8}
DSGA_ADAPTIVE_STRENGTH=${DSGA_ADAPTIVE_STRENGTH:-16.0}
DSGA_ADAPTIVE_POWER=${DSGA_ADAPTIVE_POWER:-0.5}
DSGA_NORM_RESTORE=${DSGA_NORM_RESTORE:-false}
ADD_G_LAYERWISE=${ADD_G_LAYERWISE:-false}
ADD_G_DIRONLY=${ADD_G_DIRONLY:-false}
ADD_SYM_GLOBAL=${ADD_SYM_GLOBAL:-false}

IFS=',' read -r -a SEED_ARR <<< "${SEEDS}"

mkdir -p "${OUT_DIR}" "${RUNS_ROOT}"
if [[ ! -f "${MANIFEST}" ]]; then
  echo -e "jobid\trun_name\tlabel\tseed\tstrategy\tcagrad_beta\tlaga_grouping\tdsga_m_scope\tdsga_m_align_gamma\tdsga_d_mode\tdsga_preserve_target\tdsga_layer_adaptive_blend\tdsga_layer_adaptive_strength\tdsga_layer_adaptive_power\tlaunch_cmd" > "${MANIFEST}"
fi
...
append_one() {
  ...
  local dsga_d_mode="$9"
  local dsga_preserve_target="${10}"
  local dsga_layer_adaptive_blend="${11}"
  local dsga_layer_adaptive_strength="${12}"
  local dsga_layer_adaptive_power="${13}"
  ...
  local overrides=(
    ...
    "train.dsga_d_mode=${dsga_d_mode}"
    "train.dsga_d_conflict_threshold=0.0"
    "train.dsga_d_conflict_only=false"
    "train.dsga_preserve_target=${dsga_preserve_target}"
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "" "${run_name}" "${label}" "${seed}" "${strategy}" "${cagrad_beta}" "${laga_grouping}" "${dsga_m_scope}" "${dsga_m_align_gamma}" "${dsga_d_mode}" "${dsga_preserve_target}" \
...
for seed in "${SEED_ARR[@]}"; do
  append_one "cifar100_main_naive_seed${seed}" "naive" "${seed}" "naive" "0.0" "global" "global" "0.0" "full" "understanding" "false" "0.0" "1.0"
  append_one "cifar100_main_pcgrad_seed${seed}" "pcgrad" "${seed}" "pcgrad" "0.0" "global" "global" "0.0" "full" "understanding" "false" "0.0" "1.0"
  append_one "cifar100_main_cagrad_seed${seed}" "cagrad" "${seed}" "cagrad" "${CAGRAD_BETA}" "global" "global" "0.0" "full" "understanding" "false" "0.0" "1.0"
  append_one "cifar100_main_dsga_global_seed${seed}" "dsga_global" "${seed}" "dsga" "0.0" "global" "global" "${DSGA_GAMMA}" "full" "understanding" "false" "0.0" "1.0"
  append_one "cifar100_main_dsga_layerwise_seed${seed}" "dsga_layerwise" "${seed}" "dsga" "0.0" "layerwise" "global" "${DSGA_GAMMA}" "full" "understanding" "false" "0.0" "1.0"
  append_one "cifar100_main_dsga_adaptive_seed${seed}" "dsga_adaptive" "${seed}" "dsga" "0.0" "layerwise" "global" "${DSGA_GAMMA}" "full" "understanding" "true" "${DSGA_ADAPTIVE_STRENGTH}" "${DSGA_ADAPTIVE_POWER}"
  append_one "cifar100_main_dsga_g_global_seed${seed}" "dsga_g_global" "${seed}" "dsga" "0.0" "global" "global" "${DSGA_GAMMA}" "full" "generation" "false" "0.0" "1.0"
  if [[ "${ADD_SYM_GLOBAL}" == "true" ]]; then
    append_one "cifar100_main_dsga_sym_global_seed${seed}" "dsga_sym_global" "${seed}" "dsga" "0.0" "global" "global" "${DSGA_GAMMA}" "full" "symmetric" "false" "0.0" "1.0"
  fi
  if [[ "${ADD_G_LAYERWISE}" == "true" ]]; then
    append_one "cifar100_main_dsga_g_layerwise_seed${seed}" "dsga_g_layerwise" "${seed}" "dsga" "0.0" "layerwise" "global" "${DSGA_GAMMA}" "full" "generation" "false" "0.0" "1.0"
  fi
  append_one "cifar100_main_dsga_u_dironly_seed${seed}" "dsga_u_dironly" "${seed}" "dsga" "0.0" "global" "global" "0.0" "direction_only" "understanding" "false" "0.0" "1.0"
  if [[ "${ADD_G_DIRONLY}" == "true" ]]; then
    append_one "cifar100_main_dsga_g_dironly_seed${seed}" "dsga_g_dironly" "${seed}" "dsga" "0.0" "global" "global" "0.0" "direction_only" "generation" "false" "0.0" "1.0"
  fi
done
