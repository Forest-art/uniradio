...
# Compare global CAGrad vs layer-wise CAGrad on CIFAR-10 ResNet-18.
# Default excludes consistency/supcon losses for a clean decomposition test.
...
RUN_PREFIX="${RUN_PREFIX:-study_layercagrad}"
...
for seed in "${seed_arr[@]}"; do
  ...
  submit_one "configs/cifar10_resnet18_layercagrad.yaml" "${RUN_PREFIX}_layercagrad_s${seed}" "$seed"
done

echo "[submit_layercagrad_compare] submitted seeds=${SEEDS} include_naive=${INCLUDE_NAIVE}"
