...
def main() -> None:
    ...
    parser.add_argument("--runs_glob", default="runs/sun397_dino_cmp_*")
    ...
    order = {"naive": 0, "pcgrad": 1, "cagrad": 2}
    ...
    _write_csv(
        ...
        pareto,
        ["exp_name", "strategy", "acc", "mse", "run_dir"],
    )

    fig, ax = plt.subplots(1, 1, figsize=(7.8, 5.6))
    color = {"naive": "#111111", "pcgrad": "#1f77b4", "cagrad": "#2ca02c"}
    marker = {"naive": "*", "pcgrad": "o", "cagrad": "s"}
    ...
    for r in rows:
        ...
        ax.scatter(
            [r["mse"]],
            [r["acc"]],
            s=120 if strategy == "naive" else 80,
            c=color.get(strategy, "#666666"),
            marker=marker.get(strategy, "o"),
            label=strategy,
        )
        ax.text(float(r["mse"]) + 0.001, float(r["acc"]) + 0.0002, strategy, fontsize=8)
    ...
    ax.set_title("SUN397 DINO Compare")
    ax.grid(alpha=0.25)
    ...
    print(f"[summarize_sun397_dino_compare] runs={len(rows)}")
    print(f"[summarize_sun397_dino_compare] summary={out_dir / 'summary.csv'}")
    print(f"[summarize_sun397_dino_compare] delta={out_dir / 'delta_vs_naive.csv'}")
    print(f"[summarize_sun397_dino_compare] pareto={out_dir / 'pareto_front.csv'}")
    print(f"[summarize_sun397_dino_compare] fig={fig_path}")
