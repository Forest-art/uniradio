import argparse
import csv
import glob
import json
import os
from pathlib import Path
from typing import Dict, List

import matplotlib.pyplot as plt


def _load_json(path: str) -> Dict:
    if not os.path.exists(path):
        return {}
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)


def _write_csv(path: str, rows: List[Dict], keys: List[str]) -> None:
    Path(path).parent.mkdir(parents=True, exist_ok=True)
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=keys)
        w.writeheader()
        for r in rows:
            w.writerow({k: r.get(k, "") for k in keys})


def _pareto_front(rows: List[Dict]) -> List[Dict]:
    out = []
    for i, a in enumerate(rows):
        dominated = False
        for j, b in enumerate(rows):
            if i == j:
                continue
            if (float(b["acc"]) >= float(a["acc"])) and (float(b["mse"]) <= float(a["mse"])):
                if (float(b["acc"]) > float(a["acc"])) or (float(b["mse"]) < float(a["mse"])):
                    dominated = True
                    break
        if not dominated:
            out.append(a)
    return sorted(out, key=lambda x: (float(x["mse"]), -float(x["acc"])))


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--runs_glob", required=True)
    parser.add_argument("--out_dir", required=True)
    args = parser.parse_args()

    rows: List[Dict] = []
    for rd in sorted(glob.glob(args.runs_glob)):
        if not os.path.isdir(rd):
            continue
        ts = _load_json(os.path.join(rd, "train_setup.json"))
        cs = _load_json(os.path.join(rd, "cos_summary.json"))
        u = _load_json(os.path.join(rd, "understanding.json"))
        g = _load_json(os.path.join(rd, "generation.json"))
        if not ts or not cs or not u or not g:
            continue

        rows.append(
            {
                "exp_name": os.path.basename(rd),
                "dataset": ts.get("dataset", ""),
                "strategy": str(ts.get("strategy", cs.get("strategy", "naive"))).lower(),
                "backbone": ts.get("backbone", ""),
                "lambda_txt": float(ts.get("lambda_txt", 1.0)),
                "lambda_rec": float(ts.get("lambda_rec", 1.0)),
                "cagrad_beta": float(ts.get("cagrad_beta", cs.get("cagrad_beta", 0.0)) or 0.0),
                "acc": float(u.get("zero_shot_acc", u.get("acc_txt"))),
                "mse": float(g.get("mse", g.get("recon_mse"))),
                "cos_mean": cs.get("cos_mean", ""),
                "cos_neg_ratio": cs.get("cos_neg_ratio", ""),
                "train_steps": cs.get("train_steps", ""),
                "run_dir": rd,
            }
        )

    order = {"naive": 0, "cagrad": 1, "pcgrad": 2}
    rows = sorted(rows, key=lambda x: (order.get(x["strategy"], 99), x["cagrad_beta"], x["lambda_txt"], x["exp_name"]))

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    _write_csv(
        str(out_dir / "summary.csv"),
        rows,
        [
            "exp_name",
            "dataset",
            "strategy",
            "backbone",
            "cagrad_beta",
            "lambda_txt",
            "lambda_rec",
            "acc",
            "mse",
            "cos_mean",
            "cos_neg_ratio",
            "train_steps",
            "run_dir",
        ],
    )

    naive = next((r for r in rows if r["strategy"] == "naive"), None)
    deltas: List[Dict] = []
    if naive is not None:
        for r in rows:
            if r["strategy"] == "naive":
                continue
            deltas.append(
                {
                    "exp_name": r["exp_name"],
                    "strategy": r["strategy"],
                    "cagrad_beta": r["cagrad_beta"],
                    "lambda_txt": r["lambda_txt"],
                    "lambda_rec": r["lambda_rec"],
                    "naive_acc": naive["acc"],
                    "naive_mse": naive["mse"],
                    "acc": r["acc"],
                    "mse": r["mse"],
                    "delta_acc": r["acc"] - naive["acc"],
                    "delta_mse": r["mse"] - naive["mse"],
                    "win_both_vs_naive": (r["acc"] > naive["acc"]) and (r["mse"] < naive["mse"]),
                    "run_dir": r["run_dir"],
                }
            )

    _write_csv(
        str(out_dir / "delta_vs_naive.csv"),
        deltas,
        [
            "exp_name",
            "strategy",
            "cagrad_beta",
            "lambda_txt",
            "lambda_rec",
            "naive_acc",
            "naive_mse",
            "acc",
            "mse",
            "delta_acc",
            "delta_mse",
            "win_both_vs_naive",
            "run_dir",
        ],
    )

    pareto = _pareto_front(rows)
    _write_csv(
        str(out_dir / "pareto_front.csv"),
        pareto,
        ["exp_name", "strategy", "cagrad_beta", "lambda_txt", "lambda_rec", "acc", "mse", "run_dir"],
    )

    fig, ax = plt.subplots(1, 1, figsize=(8.4, 6.0))
    for r in rows:
        strategy = r["strategy"]
        if strategy == "naive":
            ax.scatter([r["mse"]], [r["acc"]], s=140, marker="*", c="#111111", label="naive")
            ax.text(float(r["mse"]) + 0.001, float(r["acc"]) + 0.0003, "naive", fontsize=8)
        else:
            ax.scatter([r["mse"]], [r["acc"]], s=60, marker="s", c="#2ca02c", alpha=0.85, label="cagrad")
            tag = f"b={r['cagrad_beta']:.2f},lt={r['lambda_txt']:.1f}"
            ax.text(float(r["mse"]) + 0.001, float(r["acc"]) + 0.0002, tag, fontsize=7)

    if len(pareto) >= 2:
        px = [float(r["mse"]) for r in pareto]
        py = [float(r["acc"]) for r in pareto]
        ax.plot(px, py, c="#ff7f0e", lw=1.6, marker="o", ms=4, label="pareto")

    ax.set_xlabel("Generation MSE (lower is better)")
    ax.set_ylabel("Understanding Acc (higher is better)")
    ax.set_title("SUN397 + DINOv2 CAGrad Parameter Sweep")
    ax.grid(alpha=0.25)

    handles, labels = ax.get_legend_handles_labels()
    seen = set()
    hh, ll = [], []
    for h, l in zip(handles, labels):
        if l in seen:
            continue
        seen.add(l)
        hh.append(h)
        ll.append(l)
    ax.legend(hh, ll, loc="best")

    fig.tight_layout()
    fig_path = str(out_dir / "pareto_compare.png")
    fig.savefig(fig_path, dpi=180)
    plt.close(fig)

    print(f"[summarize_sun397_cagrad_sweep] runs={len(rows)}")
    print(f"[summarize_sun397_cagrad_sweep] summary={out_dir / 'summary.csv'}")
    print(f"[summarize_sun397_cagrad_sweep] delta={out_dir / 'delta_vs_naive.csv'}")
    print(f"[summarize_sun397_cagrad_sweep] pareto={out_dir / 'pareto_front.csv'}")
    print(f"[summarize_sun397_cagrad_sweep] fig={fig_path}")


if __name__ == "__main__":
    main()
